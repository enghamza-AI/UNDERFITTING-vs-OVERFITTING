#Project 3: Data Scaling Experiment
...
noise_std = 0.8
degree = 10                 # fixed high capacity — can overfit small data
...
# Large fixed validation set (used for all — fresh each time but same seed for reproducibility)
n_val = 500
...
for n_train in train_sizes:
    ...
    # Model — fixed complexity
    poly = PolynomialFeatures(degree)
    model = make_pipeline(poly, LinearRegression())
    model.fit(X_train, y_train.ravel())
    
    # Validate on large fresh hold-out
...
plt.plot(train_sizes, val_mses, 'o-', color='purple', linewidth=2.5, markersize=8)
plt.xscale('log')  # log scale makes the drop clearer
plt.xlabel('Training set size (log scale)')
plt.ylabel('Validation MSE')
plt.title('Data Scaling Experiment\nFixed high-capacity model → more data reduces overfitting')
